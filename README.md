...
- txReadUIS() код 0x10  Чтение напряжения, тока (мВ и мА) и двух байт состояния. Команда 
фоновая, запускается автоматически с периодом 0,1 секунды. Полученные данные 
выводятся на дисплей.
- txPowerAuto(spV, spI) код 0x20 Запуск ПИД-регулятора в режиме заряда с заданными максимальными уровнями напряжения и тока.
- txPowerStop() код 0x21 Команда прекратить заряд или разряд.
- txDischargeGo(spI) код 0x24 Запуск ПИД-регулятора в режиме разряда с заданным максимальным 
уровнем тока.
- txVoltageAdj(spV) код 0x25 Команда регулировки напряжения без остановки заряда.
- txCurrentAdj(spI) код 0x26 Команда регулировки тока без остановки заряда.
- txDiscurrentAdj(spD) код 0x27 Команда регулирваки тока без остановки разряда.
